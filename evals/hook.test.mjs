...
test('Unix hook recognizes dumps without executing shell syntax', async (t) => {
  ...
  const cases = [
    ...
    ['tail from second line is an EOF suffix', 'tail -n +2 large.txt', 'deny'],
    ['tail suffix with 351 remaining lines', 'tail -n +351 large.txt', 'deny'],
    ['tail suffix with 350 remaining lines', 'tail -n +352 large.txt', 'pass'],
    ['tail suffix beyond EOF', 'tail -n +702 large.txt', 'pass'],
    ['compact tail suffix option', 'tail -n+2 large.txt', 'deny'],
    ['long tail suffix option', 'tail --lines=+2 large.txt', 'deny'],
    ['tail suffix on stdin', 'tail -n +2 < large.txt', 'deny'],
    ['nested tail suffix', "bash -lc 'tail -n +2 large.txt'", 'deny'],
    ['head negative count is not a bounded prefix', 'head -n -1 large.txt', 'deny'],
    ['head at threshold', 'head -n 350 large.txt', 'pass'],
    ['head above threshold', 'head -n 351 large.txt', 'deny'],
    ['huge head count', 'head -n 50000 large.txt', 'deny'],
    ['huge compact head count', 'head -n50000 large.txt', 'deny'],
    ['huge long head count', 'head --lines=50000 large.txt', 'deny'],
    ['huge legacy head count', 'head -50000 large.txt', 'deny'],
    ['huge head count on small input', 'head -n 50000 small.txt', 'pass'],
    ['zero head count', 'head -n 0 large.txt', 'pass'],
    ['tail at threshold', 'tail -n 350 large.txt', 'pass'],
    ['tail above threshold', 'tail -n 351 large.txt', 'deny'],
    ['huge tail count', 'tail -n 50000 large.txt', 'deny'],
    ['negative tail count still selects last lines', 'tail -n -50000 large.txt', 'deny'],
    ['huge legacy tail count', 'tail -50000 large.txt', 'deny'],
    ['huge tail count on small input', 'tail -n 50000 small.txt', 'pass'],
    ['unsafe numeric count', 'head -n 9007199254740992 large.txt', 'deny'],
    ['byte prefix with few actual lines', 'head -c 351 large.txt', 'pass'],
    ['byte prefix at line threshold', 'head -c 350 blank-lines.txt', 'pass'],
    ['byte prefix above line threshold', 'head -c 351 blank-lines.txt', 'deny'],
    ['byte prefix counts its unterminated line', 'head -c 351 byte-boundary.txt', 'deny'],
    ['huge byte prefix', 'head --bytes=50000 large.txt', 'deny'],
    ['byte tail with few actual lines', 'tail -c 351 large.txt', 'pass'],
    ['byte tail counts its unterminated line', 'tail -c 350 byte-boundary.txt', 'pass'],
    ['byte tail above line threshold', 'tail -c 351 byte-boundary.txt', 'deny'],
    ['huge byte tail', 'tail --bytes=50000 large.txt', 'deny'],
    ['byte EOF suffix above line threshold', 'tail -c +2 large.txt', 'deny'],
    ['byte EOF suffix at line threshold', 'tail -c +2 blank-lines.txt', 'pass'],
    ['bounded sed', "sed -n '1,20p' large.txt", 'pass'],
    ['sed at threshold', "sed -n '2,351p' large.txt", 'pass'],
    ['sed above threshold', "sed -n '2,352p' large.txt", 'deny'],
    ['huge sed range', "sed -n '1,49999p' large.txt", 'deny'],
    ['huge sed range on small input', "sed -n '1,49999p' small.txt", 'pass'],
    ['sed range with few remaining lines', "sed -n '699,49999p' large.txt", 'pass'],
    ['sed EOF suffix above threshold', "sed -n '2,$p' large.txt", 'deny'],
    ['sed EOF suffix at threshold', "sed -n '352,$p' large.txt", 'pass'],
    ['sed global offset is not reapplied per file', "sed -n '352,$p' boundary.txt large.txt", 'deny'],
    ['reversed sed range is not a trusted window', "sed -n '352,2p' large.txt", 'deny'],
    ...
    ['awk at threshold', "awk 'NR >= 2 && NR <= 351 { print $0 }' large.txt", 'pass'],
    ['awk above threshold', "awk 'NR >= 2 && NR <= 352 { print }' large.txt", 'deny'],
    ['huge awk range', "awk 'NR <= 49999 { print }' large.txt", 'deny'],
    ['awk single late line', "awk 'NR == 500 { print }' large.txt", 'pass'],
    ['awk range with few remaining lines', "awk 'NR >= 699 && NR <= 49999' large.txt", 'pass'],
    ['awk global offset is not reapplied per file', "awk 'NR >= 352 && NR <= 49999' boundary.txt large.txt", 'deny'],
    ['full awk', "awk '{print}' large.txt", 'deny'],
    ['bounded bat', 'bat --line-range 1:20 large.txt', 'pass'],
    ['bat at threshold', 'bat --line-range=2:351 large.txt', 'pass'],
    ['bat above threshold', 'bat --line-range=2:352 large.txt', 'deny'],
    ['huge bat range', 'bat -r1:49999 large.txt', 'deny'],
    ['bat range with few remaining lines', 'bat --line-range 699:49999 large.txt', 'pass'],
    ['repeated bat ranges are not one small window', 'bat -r1:200 -r300:500 large.txt', 'deny'],
    ['reversed bat range is not a trusted window', 'bat --line-range 352:2 large.txt', 'deny'],
...
test('PowerShell command recognition treats quoted data and real pipelines separately', async (t) => {
  ...
  const cases = [
    ...
    ['total count at threshold', 'Get-Content large.txt -TotalCount 350', 'pass'],
    ['total count above threshold', 'Get-Content large.txt -TotalCount 351', 'deny'],
    ['huge total count', 'Get-Content large.txt -TotalCount 49999', 'deny'],
    ['huge head alias', 'Get-Content large.txt -Head 49999', 'deny'],
    ['huge first alias', 'Get-Content large.txt -First 49999', 'deny'],
    ['huge tail', 'Get-Content large.txt -Tail 49999', 'deny'],
    ['huge last alias', 'Get-Content large.txt -Last 49999', 'deny'],
    ['gc numeric limit', 'gc large.txt -TotalCount 49999', 'deny'],
    ['huge total count on small input', 'Get-Content small.txt -TotalCount 49999', 'pass'],
    ['unsafe total count', 'Get-Content large.txt -TotalCount 9007199254740992', 'deny'],
...
test('recognized shell windows use the configured line threshold', async (t) => {
  const { root } = await hookFiles(t);
  for (const [tool, command, decision] of [
    ['bash', 'head large.txt', 'deny'],
    ['bash', 'tail large.txt', 'deny'],
    ['bash', 'head -n 3 large.txt', 'pass'],
    ['bash', 'head -n 4 large.txt', 'deny'],
    ['bash', 'tail -n 4 large.txt', 'deny'],
    ['bash', 'tail -n +699 large.txt', 'pass'],
    ['bash', 'tail -n +698 large.txt', 'deny'],
    ['bash', "sed -n '2,4p' large.txt", 'pass'],
    ['bash', "sed -n '2,5p' large.txt", 'deny'],
    ['bash', "awk 'NR <= 4' large.txt", 'deny'],
    ['bash', 'bat -r1:4 large.txt', 'deny'],
    ['bash', 'head -c 4 large.txt', 'pass'],
    ['bash', 'head -c 4 blank-lines.txt', 'deny'],
    ['powershell', 'Get-Content large.txt -TotalCount 3', 'pass'],
    ['powershell', 'Get-Content large.txt -TotalCount 4', 'deny'],
    ['powershell', 'Get-Content large.txt -Tail 4', 'deny'],
  ]) {
    await t.test(`${tool}: ${command}`, async () => {
      assertDecision(await runHook(root, hookPayload(root, tool, { command }), {
        env: { TOKENREDUCER_LINE_THRESHOLD: '3' },
      }), decision);
    });
  }
});
...
test('the actual PowerShell launcher preserves stdin JSON and decisions', async (t) => {
  ...
  const cases = [
    ...
    ['Get-Content oversized limit deny', hookPayload(root, 'powershell', { command: 'Get-Content large.txt -TotalCount 49999' }), 'deny'],
    ['Get-Content bounded limit allow', hookPayload(root, 'powershell', { command: 'Get-Content large.txt -TotalCount 350' }), 'pass'],
